angular.module('page', ["ideUI", "ideView", "entityApi"])
	...
	.controller('PageController', ['$scope', '$http', 'messageHub', 'entityApi', 'Extensions', function ($scope, $http, messageHub, entityApi, Extensions) {
		...
		Extensions.get('dialogWindow', 'codbex-payrolls-custom-action').then(function (response) {
			$scope.pageActions = response.filter(e => e.perspective === "Payrolls" && e.view === "PayrollEntry" && (e.type === "page" || e.type === undefined));
		});
		...
		$scope.loadPage = function (pageNumber, filter) {
			...
			entityApi.count(filter).then(function (response) {
				if (response.status != 200) {
					messageHub.showAlertError("PayrollEntry", `Unable to count PayrollEntry: '${response.message}'`);
					return;
				}
				if (response.data) {
				...
				entityApi.search(filter).then(function (response) {
					if (response.status != 200) {
						messageHub.showAlertError("PayrollEntry", `Unable to list/filter PayrollEntry: '${response.message}'`);
						return;
					}
		...
		$scope.deleteEntity = function () {
			...
			messageHub.showDialogAsync(
				'Delete PayrollEntry?',
				`Are you sure you want to delete PayrollEntry? This action cannot be undone.`,
				[{
					id: "delete-btn-yes",
					type: "emphasized",
					label: "Yes",
				...
				if (msg.data === "delete-btn-yes") {
					entityApi.delete(id).then(function (response) {
						if (response.status != 204) {
							messageHub.showAlertError("PayrollEntry", `Unable to delete PayrollEntry: '${response.message}'`);
							return;
						}
						refreshData();
						$scope.loadPage($scope.dataPage, $scope.filter);
