...
export class PayrollStatusRepository {
    ...
    private async triggerEvent(data: PayrollStatusEntityEvent | PayrollStatusUpdateEntityEvent) {
        const triggerExtensions = await extensions.loadExtensionModules("codbex-payrolls-entities-PayrollStatus", ["trigger"]);
        ...
        producer.topic("codbex-payrolls-entities-PayrollStatus").send(JSON.stringify(data));
    }
}
